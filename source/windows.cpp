...
namespace Windows
{
    ...
    void ShowProgressDialog()
    {
        if (activity_inprogess)
        {
            ...
            static bool cancel_confirm = false;
            ...
            if (ImGui::BeginPopupModal(lang_strings[STR_PROGRESS], NULL, ImGuiWindowFlags_AlwaysAutoResize))
            {
                ...
                static double inst_speed = 0.0f;
                static double smoothed_speed = 0.0f;
                static char progress_text[64];
                static uint64_t cur_tick = 0;
                static double tick_delta = 0.0;
                ...
                if (file_transfering)
                {
                    ...
                    // Average progress over entire transfer.
                    progress = (bytes_to_download > 0)
                                   ? (float)bytes_transfered / (float)bytes_to_download
                                   : 0.0f;

                    // Detect a fresh file (bytes reset) so we don't carry the old speed over.
                    bool new_transfer = (bytes_transfered < last_bytes);
                    if (last_tick == 0 || new_transfer)
                    {
                        last_tick = cur_tick;
                        last_bytes = bytes_transfered;
                        inst_speed = 0.0;
                        smoothed_speed = 0.0;
                    }
                    double window_sec = (cur_tick - last_tick) * 1.0f / 1000000.0f;
                    int64_t delta_bytes = bytes_transfered - last_bytes;
                    if (window_sec >= 0.3f)
                    {
                        inst_speed = (delta_bytes > 0) ? (delta_bytes / window_sec) / 1048576.0 : 0.0; // MB/s
                        last_tick = cur_tick;
                        last_bytes = bytes_transfered;
                    }
                    // Smooth the speed a bit to avoid wild swings.
                    smoothed_speed = (smoothed_speed * 0.7) + (inst_speed * 0.3);

                    double elapsed_sec = (cur_tick - prev_tick) * 1.0 / 1000000.0;
                    double avg_speed = (elapsed_sec > 0.0) ? (bytes_transfered / elapsed_sec) / 1048576.0 : 0.0;

                    snprintf(progress_text, sizeof(progress_text), "cur %.2f MB/s | avg %.2f MB/s", smoothed_speed, avg_speed);
                    ImGui::ProgressBar(progress, ImVec2(505, 0), progress_text);
                }
                else
                {
                    // Reset progress tracking when no transfer is active to avoid stale speeds.
                    progress = 0.0f;
                    inst_speed = 0.0;
                    smoothed_speed = 0.0;
                    progress_text[0] = '\0';
                ...
                if (cancel_confirm)
                {
                    ImGui::OpenPopup("ConfirmCancelTransfer");
                    ImGui::SetNextWindowPos(ImVec2(400, 300));
                    if (ImGui::BeginPopupModal("ConfirmCancelTransfer", NULL, ImGuiWindowFlags_AlwaysAutoResize))
                    {
                        ImGui::TextWrapped("%s", lang_strings[STR_CANCEL_ACTION_MSG]);
                        ImGui::Separator();
    ...
    void DrawCreditsOverlay()
    {
        ...
        ImVec2 text_size = ImGui::CalcTextSize(credit);
        ImVec2 text_pos(win_pos.x + win_size.x - text_size.x - 12.0f,
                        win_pos.y + win_size.y - text_size.y - 8.0f);

        ImDrawList *draw = ImGui::GetWindowDrawList();
        ImU32 color = ImGui::ColorConvertFloat4ToU32(ImVec4(0.82f, 0.22f, 1.0f, 1.0f));
        draw->AddText(text_pos, color, credit);
    }
